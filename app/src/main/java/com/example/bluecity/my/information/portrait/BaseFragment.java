...
public class BaseFragment extends Fragment {
        ...
        switch (requestCode) {
            case mRequestCode: {
                for (int i = 0; i < grantResults.length; ++i) {
                    if (grantResults[i] == PackageManager.PERMISSION_DENIED) {  //第i个权限被拒绝
                        hasAllGranted = false;
                        ...
                        if (!ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), permissions[i])) {
                            new AlertDialog.Builder(getContext()).setTitle("PermissionTest")   //设置对话框标题
                                    ...
                                    }).setNegativeButton("取消", new DialogInterface.OnClickListener() {//一个消极一个取消？
                                @Override
    ...
    public void requestPermissions(final Context context, final String[] permissions, RequestPermissionCallBack callBack) {
        ...
        for (final String permission : permissions) {
            if (ContextCompat.checkSelfPermission(context, permission) == PackageManager.PERMISSION_DENIED) {   //某一权限被拒绝
                isAllGranted = false;
                if (ActivityCompat.shouldShowRequestPermissionRationale((Activity) context, permission)) {  //应该提示原因：拒绝过但没勾选“不再提示”
